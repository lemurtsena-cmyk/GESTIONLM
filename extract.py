...
# --- PRODUITS ---
    ...
    # Ajout d'une colonne pour faciliter la modification/suppression
    df['Actions'] = ''
            ...
            with st.form("add_prod"):
                ...
                if st.form_submit_button("Enregistrer"):
                    ...
                    try:
                        ...
                        st.error("Erreur : Le code du produit existe déjà.")
                    except Exception as e:
                        st.error(f"Erreur lors de l'enregistrement : {e}")

    with col2:
        with st.expander("✏️ Modifier/Supprimer Produit"):
            st.subheader("Sélectionnez un produit pour modifier ou supprimer")
            # Créer un DataFrame pour la sélection
            products_for_selection = df[['id', 'code', 'nom']].copy()
            selected_id = st.selectbox("Sélectionner le produit", products_for_selection.itertuples(), format_func=lambda x: f"{x.nom} ({x.code})")

            if selected_id:
                product_to_edit = df[df['id'] == selected_id.id].iloc[0]
                with st.form(f"edit_prod_{product_to_edit['id']}"):
                    st.write(f"Modification du produit : **{product_to_edit['nom']}**")
                    c1,c2,c3 = st.columns(3)
                    cat = c1.selectbox("Catégorie", ["TABLE", "CHAISE", "BUREAU", "ETAGERE", "AUTRE"], index=LISTE_COULEURS.index(product_to_edit['categorie']) if product_to_edit['categorie'] in LISTE_COULEURS else 0) # Correction: index should be for categories
                    coul = c2.selectbox("Couleur", LISTE_COULEURS, index=LISTE_COULEURS.index(product_to_edit['couleur']) if product_to_edit['couleur'] in LISTE_COULEURS else 0)
                    pieds = c3.selectbox("Forme pieds", LISTE_PIEDS, index=LISTE_PIEDS.index(product_to_edit['forme_pieds']) if product_to_edit['forme_pieds'] in LISTE_PIEDS else 0)
                    long = c1.number_input("Longueur (cm)", value=int(product_to_edit['longueur']))
                    larg = c2.number_input("Largeur (cm)", value=int(product_to_edit['largeur']))
                    haut = c3.number_input("Hauteur (cm)", value=int(product_to_edit['hauteur']))
                    pa = st.number_input("Prix d'achat (Ar)", value=int(product_to_edit['prix_achat']))
                    pv = st.number_input("Prix de vente (Ar)", value=int(product_to_edit['prix_vente']))
                    stock = st.number_input("Stock", value=int(product_to_edit['stock'])) # Allow modifying stock directly here
                    ...
                    with col_update:
                        if st.form_submit_button("Mettre à jour"):
                            conn.execute("""
                                UPDATE produits
                                SET categorie = ?, hauteur = ?, longueur = ?, largeur = ?, couleur = ?,
                                    forme_pieds = ?, prix_achat = ?, prix_vente = ?, stock = ?
                                WHERE id = ?
                            """, (cat, haut, long, larg, coul, pieds, pa, pv, stock, product_to_edit['id']))
                            conn.commit()
                    ...
                    with col_delete:
                        if st.form_submit_button("Supprimer"):
                            # Confirmation dialog
                            if st.checkbox("Confirmer la suppression de ce produit et de tous les mouvements associés"):
                                try:
                                    conn.execute("DELETE FROM mouvements WHERE produit_id = ?", (product_to_edit['id'],))
                                    conn.execute("DELETE FROM produits WHERE id = ?", (product_to_edit['id'],))
                                    conn.commit()
...
# --- ENTREES STOCK ---
    ...
    with st.form("entree"):
        ...
        if st.form_submit_button("Valider l'achat"):
            ...
            # Update prix_achat with the latest purchase price, or average if preferred
            # For simplicity, using latest purchase price here. Consider averaging if needed.
    ...
    with col1:
        with st.form("cmd_form"):
            ...
            if st.form_submit_button("Enregistrer la commande"):
                ...
                if avance > 0: # Only add to journal if an advance was paid
        ...
        if not df_c.empty:
            ...
            with st.expander("Supprimer une commande"):
                ...
                if st.button("Confirmer Suppression"):
                    ...
                    command_exists = conn.execute("SELECT COUNT(*) FROM commandes WHERE id=?", (id_del,)).fetchone()[0]
                    if command_exists:
                        conn.execute("DELETE FROM commandes WHERE id=?", (id_del,))
                        conn.commit(); st.rerun()
                    else:
                        st.warning("Commande avec cet ID n'existe pas.")
...
# --- JOURNALIER ---
    ...
    with col2:
        ...
        df_j = pd.read_sql("SELECT id, date, type, description, mode, montant FROM journal ORDER BY date DESC", conn)
        st.dataframe(df_j, use_container_width=True)

        with st.expander("Modifier/Supprimer une opération"):
            st.subheader("Sélectionnez une opération pour modifier ou supprimer")
            selected_journal_id = st.number_input("ID de l'opération à modifier/supprimer", min_value=1, step=1)

            if selected_journal_id:
                operation = pd.read_sql("SELECT * FROM journal WHERE id = ?", conn, params=(selected_journal_id,)).iloc[0]
                with st.form(f"edit_journal_{operation['id']}"):
                    st.write(f"Modification de l'opération ID: **{operation['id']}**")
                    typ = st.selectbox("Type", ["DEPENSE", "RECETTE"], index=["DEPENSE", "RECETTE"].index(operation['type']))
                    des = st.text_input("Description / Motif", value=operation['description'])
                    mnt = st.number_input("Montant (Ar)", value=int(operation['montant']))
                    mode = st.selectbox("Mode", LISTE_MODES, index=LISTE_MODES.index(operation['mode']))
                    dat = st.date_input("Date", value=datetime.fromisoformat(operation['date']))
                    ...
                    with col_update:
                        if st.form_submit_button("Mettre à jour"):
                            conn.execute("""
                                UPDATE journal
                                SET date = ?, type = ?, description = ?, montant = ?, mode = ?
                                WHERE id = ?
                            """, (dat.isoformat(), typ, des, mnt, mode, selected_journal_id))
                            conn.commit()
                            st.success("Opération mise à jour !")
                            st.rerun()
                    with col_delete:
                        if st.form_submit_button("Supprimer"):
                            if st.checkbox(f"Confirmer la suppression de l'opération ID {selected_journal_id}"):
                                conn.execute("DELETE FROM journal WHERE id = ?", (selected_journal_id,))
                                conn.commit()
...
# --- COMPTABILITÉ ---
    ...
    # Combine mouvements and journal for a complete transaction history
    mouv_df = pd.read_sql("SELECT date, type, (qte*pu) as mnt, mode FROM mouvements", conn)
    jour_df = pd.read_sql("SELECT date, type, montant as mnt, mode FROM journal", conn)

    # Ensure 'mnt' column is numeric and handle potential NaNs
    mouv_df['mnt'] = pd.to_numeric(mouv_df['mnt'], errors='coerce').fillna(0)
    jour_df['mnt'] = pd.to_numeric(jour_df['mnt'], errors='coerce').fillna(0)

    all_trans = pd.concat([mouv_df, jour_df])

    # Define 'RECETTE' types from both tables
    RECETTE_TYPES = ['VENTE', 'RECETTE']
    DEPENSE_TYPES = ['ACHAT', 'DEPENSE']

    st.subheader("🏦 Solde par mode de paiement")
    cols = st.columns(len(LISTE_MODES))
    for i, m in enumerate(LISTE_MODES):
        rec = all_trans[(all_trans.mode == m) & (all_trans.type.isin(RECETTE_TYPES))]['mnt'].sum()
        dep = all_trans[(all_trans.mode == m) & (all_trans.type.isin(DEPENSE_TYPES))]['mnt'].sum()
        solde = rec - dep
        cols[i].metric(m, mga(solde))

    st.divider()
    recettes_total = all_trans[all_trans.type.isin(RECETTE_TYPES)]['mnt'].sum()
    depenses_total = all_trans[all_trans.type.isin(DEPENSE_TYPES)]['mnt'].sum()

    c1,c2,c3 = st.columns(3)
    c1.metric("Total Recettes", mga(recettes_total))
